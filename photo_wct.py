...
class PhotoWCT(nn.Module):
    def __init__(self, args):
        ...
        if self.args.mode == "" or self.args.mode == "original":
          #### original model
          self.e1 = VGGEncoder(1); self.d1 = VGGDecoder(1)
          self.e2 = VGGEncoder(2); self.d2 = VGGDecoder(2)
          self.e3 = VGGEncoder(3); self.d3 = VGGDecoder(3)
          self.e4 = VGGEncoder(4); self.d4 = VGGDecoder(4)
    ...
    def transform(self, cont_img, styl_img, cont_seg, styl_seg):
        ...
        if self.args.mode == "" or self.args.mode == "original":
          ...
          #### slimmed model
          sF4 = self.e4(styl_img); torch.cuda.empty_cache()
          cF4, cpool_idx, cpool1, cpool_idx2, cpool2, cpool_idx3, cpool3 = self.e4.forward_stem(cont_img)[3:]; torch.cuda.empty_cache()
          sF4 = sF4.data.squeeze(0)
          cF4 = cF4.data.squeeze(0)
          csF4 = self.__feature_wct(cF4, sF4, cont_seg, styl_seg);
          Im4 = self.d4(csF4, cpool_idx, cpool1, cpool_idx2, cpool2, cpool_idx3, cpool3); torch.cuda.empty_cache()
          
          sF3 = self.e3(styl_img); torch.cuda.empty_cache()
          cF3, cpool_idx, cpool1, cpool_idx2, cpool2 = self.e3.forward_stem(cont_img)[2:]; torch.cuda.empty_cache()
          sF3 = sF3.data.squeeze(0) 
          cF3 = cF3.data.squeeze(0)
          csF3 = self.__feature_wct(cF3, sF3, cont_seg, styl_seg)
          Im3 = self.d3(csF3, cpool_idx, cpool1, cpool_idx2, cpool2); torch.cuda.empty_cache()
          
          sF2 = self.e2(styl_img); torch.cuda.empty_cache()
          cF2, cpool_idx, cpool = self.e2.forward_stem(Im3)[1:]; torch.cuda.empty_cache()
          sF2 = sF2.data.squeeze(0)
          cF2 = cF2.data.squeeze(0)
          csF2 = self.__feature_wct(cF2, sF2, cont_seg, styl_seg)
          Im2 = self.d2(csF2, cpool_idx, cpool); torch.cuda.empty_cache()
          
          sF1 = self.e1(styl_img); torch.cuda.empty_cache()
          cF1 = self.e1(Im2); torch.cuda.empty_cache()
          sF1 = sF1.data.squeeze(0)
          cF1 = cF1.data.squeeze(0)
          csF1 = self.__feature_wct(cF1, sF1, cont_seg, styl_seg)
          Im1 = self.d1(csF1); torch.cuda.empty_cache()
        return Im1
